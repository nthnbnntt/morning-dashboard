...
async function trackPage(env: Env, path: string, elapsed: number): Promise<void> {
  ...
  await metric(env, "page_loads");
  await metric(env, `page:${label}`);
  await metric(env, "response_samples");
  await metric(env, "last_response_ms", Math.round(elapsed));
  await event(env, `${label} page loaded`);
}
...
async function metrics(env: Env): Promise<Record<string, unknown>> {
  const { results: rows } = await env.DB.prepare("SELECT key, value FROM metrics").all<{ key: string; value: number }>();
  const values = Object.fromEntries(rows.map((row) => [row.key, row.value]));
  const open = await env.DB.prepare("SELECT COUNT(*) AS count FROM tasks WHERE done = 0").first<{ count: number }>();
  const complete = await env.DB.prepare("SELECT COUNT(*) AS count FROM tasks WHERE done = 1").first<{ count: number }>();
  const stories = await env.DB.prepare("SELECT COUNT(*) AS count FROM unique_items WHERE type = 'story'").first<{ count: number }>();
  const releases = await env.DB.prepare("SELECT COUNT(*) AS count FROM unique_items WHERE type = 'release'").first<{ count: number }>();
  const company = await env.DB.prepare("SELECT COUNT(*) AS count FROM unique_items WHERE type = 'company'").first<{ count: number }>();
  const { results: events } = await env.DB.prepare("SELECT label, amount, created_at FROM metric_events ORDER BY id DESC LIMIT 12").all();
  return {
    ...values,
...
async function newsPage(env: Env): Promise<string> {
  ...
  const body = `
    <section class="panel">
      <div class="status" data-state="${escapeHtml(status.state)}"><span class="status-dot"></span><strong>Quickbase ${escapeHtml(status.label)}</strong><span class="meta">${escapeHtml(status.detail)}</span><a href="${escapeHtml(status.url)}" target="_blank" rel="noreferrer">Open</a></div>
      <a class="card" href="tasks.html"><span class="meta">Tasks</span><h2>${openTasks}</h2></a>
      <div class="source-list">${sources}</div>
    </section>
    ${lead ? `<section class="lead story" data-source="${escapeHtml(lead.source)}"><span class="meta">${escapeHtml(lead.source)} / ${escapeHtml(lead.published)}</span><h2><a href="${escapeHtml(lead.url)}" target="_blank" rel="noreferrer">${escapeHtml(lead.title)}</a></h2><p>${escapeHtml(shorten(lead.summary, 300))}</p></section>` : ""}
...
async function companyPage(): Promise<string> {
  const body = `<section class="panel"><h2>Company News</h2><p id="company-status">Loading company news...</p><a href="${VTG_NEWS_URL}" target="_blank" rel="noreferrer">Open source page</a></section><section class="grid two" id="company-list"></section>`;
...
function listPageScript(api: string, kind: "release" | "company"): string {
  if (kind === "release") {
    return `
let records=[],filter="all";function esc(v){return String(v||"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]))}function statusOf(r){return String(r.status||"").toLowerCase().includes("released")?"released":"upcoming"}function show(r){if(filter==="all")return true;if(filter==="new")return r.isNew;if(filter==="released")return statusOf(r)==="released";return statusOf(r)==="upcoming"}function render(){document.getElementById("release-list").innerHTML=records.filter(show).map(r=>'<article class="card"><span class="pill '+statusOf(r)+'">'+esc(statusOf(r))+'</span>'+(r.isNew?'<span class="pill new">New</span>':"")+'<h3>'+esc(r.feature||r.summary||"Quickbase release")+'</h3><div class="meta">'+esc(r.area||"")+'</div><p>'+esc(r.summary||"")+'</p><a href="'+esc(r.url)+'" target="_blank" rel="noreferrer">View note</a></article>').join("")||'<div class="empty">No release notes match this filter.</div>'}document.querySelectorAll(".filters button").forEach(b=>b.onclick=()=>{filter=b.dataset.filter;document.querySelectorAll(".filters button").forEach(x=>x.classList.toggle("active",x===b));render()});fetch("${api}").then(r=>r.json()).then(d=>{records=d.records||[];const n=records.filter(r=>r.isNew).length;document.getElementById("release-status").textContent=records.length+" release notes loaded / "+n+" new. Updated "+d.generated+".";render()});
`;
  }
  return `
function esc(v){return String(v||"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]))}fetch("${api}").then(r=>r.json()).then(d=>{document.getElementById("company-status").textContent=(d.records||[]).length+" company stories loaded. Updated "+d.generated+".";document.getElementById("company-list").innerHTML=(d.records||[]).map(r=>'<article class="card">'+(r.isNew?'<span class="pill new">New</span>':"")+'<div class="meta">'+esc(r.date)+'</div><h3>'+esc(r.title)+'</h3><p>'+esc(r.summary)+'</p><a href="'+esc(r.url)+'" target="_blank" rel="noreferrer">Read more</a></article>').join("")});
`;
}

function metricsPage(): string {
  const body = `<section class="panel"><h2>Live Instrumentation</h2><p id="metrics-status">Loading dashboard metrics...</p><button class="refresh" id="refresh">Refresh</button></section><section class="grid" id="gauges"></section><section class="grid two" id="stats"></section><section class="grid two"><div class="card"><h3>Recent Events</h3><div id="events"></div></div></section>`;
  const script = `
function fmt(v){return Math.round(Number(v||0)).toLocaleString()}function gauge(l,v,max){const pct=max?Math.max(0,Math.min(100,Number(v||0)/max*100)):0;return '<article class="card"><div class="gauge" style="--pct:'+pct.toFixed(1)+'"><strong>'+fmt(v)+'</strong></div><h3>'+l+'</h3></article>'}function stat(l,v){return '<article class="card"><span class="meta">'+l+'</span><h2>'+fmt(v)+'</h2></article>'}async function load(){const d=await(await fetch("/api/metrics")).json();const max=Math.max(100,d.page_loads||0,d.unique_stories_served||0,d.unique_release_notes_loaded||0,d.unique_company_news_loaded||0);document.getElementById("gauges").innerHTML=[gauge("Page Loads",d.page_loads,max),gauge("Unique Stories",d.unique_stories_served,max),gauge("Unique Releases",d.unique_release_notes_loaded,max),gauge("Unique Company News",d.unique_company_news_loaded,max)].join("");document.getElementById("stats").innerHTML=[stat("Tasks Created",d.tasks_created),stat("Tasks Completed",d.tasks_completed),stat("Tasks Deleted",d.tasks_deleted),stat("Open Tasks",d.current_open_tasks),stat("Completed Tasks",d.current_completed_tasks),stat("News Refreshes",d.news_refreshes)].join("");document.getElementById("events").innerHTML=(d.events||[]).map(e=>'<p><strong>'+e.label+'</strong> <span class="meta">'+e.amount+' / '+e.created_at+'</span></p>').join("")||'<p>No events yet.</p>';document.getElementById("metrics-status").textContent="Updated "+d.generated+"."}document.getElementById("refresh").onclick=load;load();setInterval(load,60000);
`;
...
async function route(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if (path === "/api/tasks") {
    if (request.method === "GET") return json(await tasks(env));
  ...
  if (path === "/api/quickbase-status") return json(await quickbaseStatus());
