...
async function newsPage(env: Env): Promise<string> {
  const [stories, allTasks, status] = await Promise.all([cachedStories(env), tasks(env), quickbaseStatus()]);
  ...
  const body = `
    <section class="panel">
      <div class="status" data-state="${escapeHtml(status.state)}"><span class="status-dot"></span><strong>Quickbase ${escapeHtml(status.label)}</strong><span class="meta">${escapeHtml(status.detail)}</span><a href="${escapeHtml(safeUrl(status.url))}" target="_blank" rel="noreferrer">Open</a></div>
